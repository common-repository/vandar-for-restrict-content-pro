...
function rcp_vandar_create_payment( $subscription_data ) {
	...
	global $rcp_options;
	// wp_die( __( 'Vandar API key is missing' ) );
	...
	if ( is_wp_error( $response ) ) {
		rcp_errors()->add( 'vandar_error', sprintf( __( 'Unfortunately, the payment couldn\'t be processed due to the following reason: %s' ), $response->get_error_message() ), 'register' );
		return;
	}
	...
	if ( 200 !== $http_status || empty( $result ) || empty( $result->token ) || $result->status == 0) {
		...
		rcp_errors()->add( 'vandar_error', sprintf( __( 'Unfortunately, the payment couldn\'t be processed due to the following reason: %s' ), $msg ), 'register' );
		return;
	}
